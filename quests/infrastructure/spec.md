...
active: [self]
activity: plan
next: select the next unit; M2 is the intended follow-on and shares M1's fixtures and helpers
waiting_on: next-unit-selection
profile: current owner-selected session; no unit is executing
---
...
- [Playwright configuration](../../playwright.config.ts) already starts docs on port 5173 and the [isolated picker fixture](../../tests/fixtures/vite.config.ts) on 5174. [Picker helpers](../../tests/components/date-picker-helpers.ts) collect page errors and control the clock. Reuse this infrastructure and retain useful docs smoke cases.
...
|---|---|---|
| M1: minimal verification groundwork | **Delivered and accepted 2026-09-20.** Component tests moved to owned fixtures, docs smoke isolated, shared runtime-error handling, failure diagnostics and the button accessibility scenario. [Bounded brief and evidence](work/minimal-verification.md). | All three engines pass in [CI run 35521385395](https://github.com/rashfael/buntpapier/actions/runs/35521385395). Commands and fixture usage live in [the testing guide](../../design/testing.md). Reduced-motion handling stays unmet and is M4's. |
