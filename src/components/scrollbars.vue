...
<template lang="pug">
.bunt-scrollbars
	.bunt-scroll-content(ref="contentRef", @scroll="handleScroll")
		slot
	template(v-for="dim of Object.keys(dimensions)", :key="dim")
		div(:class="[`bunt-scrollbar-rail-${dim}`, {active: draggingDimension === dim}]", @pointerdown="handlePointerdown(dim, $event)")
			.bunt-scrollbar-thumb(:ref="el => { thumbRefs[dim] = el }", :style="thumbStyles[dim]")
</template>
<style lang="sass">
$rail-width: 15px
$thumb-width: 6px
