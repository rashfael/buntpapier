...
**A fixture is one `.vue` file.** A plugin in `tests/fixtures/vite.config.ts` generates the page around it: a request for `/<name>` (or `/<name>.html`) is matched against the `.vue` files present, served as the shared HTML through `transformIndexHtml`, and mounted by an inline module that imports `mount.ts` and the component. Names match case-insensitively and ignore dashes, `/` lists the available fixtures, and an unknown name returns a 404 naming them. Only the dev server does this; nothing builds these pages.
...
- The button scan asserts that `results.violations` is empty, preserving the full axe diagnostics on failure. Add exception matching only when an accepted finding needs it, scoped to its rule and targets with an owner and removal condition.

One criterion is explicitly **unmet**: item 8 of the [accessibility acceptance](accessibility.md) list asks for `prefers-reduced-motion` handling, and the library has none — `src/` contains no `prefers-reduced-motion` rule. The reduced-motion case in `button-a11y.test.ts` checks activation, blocks repeat activation while a controllable promise is pending, and verifies that the button accepts another activation after completion and feedback. This checks operation under emulation; M4 still owns motion suppression. The forced-colours case in the same file is a real guard: `forced-color-adjust` must not become `none`. It reads the property rather than the camelCase IDL alias, which WebKit does not expose; where an engine does not implement the property the read is `''` and the guard is vacuous rather than wrong.
